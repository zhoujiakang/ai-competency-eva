...
public class UserController {
 ...
 public record Profile(String name,String nickname,String phone,String email){}
 ...
 @PutMapping("/me") public ApiResponse<?> update(@RequestBody Profile p){User u=current();if(p.name()!=null&&!p.name().isBlank())u.setName(p.name());if(p.nickname()!=null&&!p.nickname().isBlank())u.setNickname(p.nickname());u.setUpdatedAt(Instant.now());return ApiResponse.ok(profile(users.save(u)));}
 ...
 private Map<String,Object> profile(User u){Map<String,Object> p=new HashMap<>();p.put("id",u.getId());p.put("account",u.getUsername());p.put("name",u.getName());p.put("nickname",u.getNickname());p.put("registeredAt",u.getCreatedAt());return p;}
}
