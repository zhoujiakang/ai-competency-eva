...
import com.huiqiyikang.assessment.entity.Question; import com.huiqiyikang.assessment.service.QuestionService;
import cn.dev33.satoken.stp.StpUtil; import com.fasterxml.jackson.core.JsonProcessingException; import com.fasterxml.jackson.databind.ObjectMapper; import com.huiqiyikang.assessment.common.*; import jakarta.validation.Valid; import jakarta.validation.constraints.*; import org.springframework.web.bind.annotation.*; import java.time.Instant; import java.util.List;
@RestController @RequestMapping("/api/questions") public class QuestionController {
 private final QuestionService repo; private final ObjectMapper mapper; public QuestionController(QuestionService r,ObjectMapper mapper){repo=r;this.mapper=mapper;}
 ...
 @GetMapping("/public") public ApiResponse<?> publicly(){return ApiResponse.ok(repo.findByVisibilityAndStatus("public","active"));}
 /**
 ...
 @PostMapping public ApiResponse<?> create(@Valid @RequestBody Request r){validate(r);Question q=new Question();q.setOwnerUserId(StpUtil.getLoginIdAsLong());apply(q,r);return ApiResponse.ok(repo.save(q));}
 @GetMapping("/{id}") public ApiResponse<?> detail(@PathVariable Long id){return ApiResponse.ok(repo.findById(id).orElseThrow(()->new BusinessException("题目不存在")));}
 @PutMapping("/{id}") public ApiResponse<?> update(@PathVariable Long id,@Valid @RequestBody Request r){validate(r);Question q=owned(id);apply(q,r);q.setUpdatedAt(Instant.now());return ApiResponse.ok(repo.save(q));}
 @PostMapping("/{id}/publish") public ApiResponse<?> publish(@PathVariable Long id){Question q=owned(id);q.setVisibility("public");q.setUpdatedAt(Instant.now());return ApiResponse.ok(repo.save(q));}
 @PostMapping("/{id}/unpublish") public ApiResponse<?> unpublish(@PathVariable Long id){Question q=owned(id);q.setVisibility("private");q.setUpdatedAt(Instant.now());return ApiResponse.ok(repo.save(q));}
 @PostMapping("/{id}/offline") public ApiResponse<?> offline(@PathVariable Long id){Question q=owned(id);q.setStatus("offline");q.setUpdatedAt(Instant.now());return ApiResponse.ok(repo.save(q));}
 @PostMapping("/{id}/restore") public ApiResponse<?> restore(@PathVariable Long id){Question q=owned(id);q.setStatus("active");q.setUpdatedAt(Instant.now());return ApiResponse.ok(repo.save(q));}
 @PostMapping("/public/{id}/copy") public ApiResponse<?> copy(@PathVariable Long id){Question source=repo.findById(id).orElseThrow(()->new BusinessException("题目不存在"));if(!"public".equals(source.getVisibility())||!"active".equals(source.getStatus()))throw new BusinessException("题目当前不可复制");Question q=new Question();q.setOwnerUserId(StpUtil.getLoginIdAsLong());q.setType(source.getType());q.setTitle(source.getTitle());q.setContent(source.getContent());q.setOptions(source.getOptions());q.setAnswer(source.getAnswer());q.setRubric(source.getRubric());q.setTags(source.getTags());q.setAssessmentPoints(source.getAssessmentPoints());q.setDifficulty(source.getDifficulty());q.setScore(source.getScore());return ApiResponse.ok(repo.save(q));}
